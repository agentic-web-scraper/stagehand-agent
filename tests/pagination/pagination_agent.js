...
console.log("🤖 AI-Powered Pagination Agent");
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      try {
        ...
        if (pageNum < maxPages) {
          ...
          // Check if More link exists using Playwright first
          const moreExists = await page.locator('a:has-text("More")').count() > 0;
        ...
        if (error.message.includes('Failed to parse model response as JSON')) {
          ...
          try {
            ...
            console.log(`  ❌ Retry failed, moving to next page\n`);
          }
        } else {
          break;
        }
