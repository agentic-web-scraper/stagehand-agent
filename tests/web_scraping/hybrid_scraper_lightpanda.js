...
USAGE:
  node hybrid_scraper_lightpanda.js [OPTIONS]
...
EXAMPLES:
  ...
  node hybrid_scraper_lightpanda.js \\
...
async function extractWithSelectors(page, selectors) {
  ...
  const data = await page.evaluate((sel) => {
    ...
    // Helper to get elements by XPath or CSS
    ...
    const getElement = (parent, selector) => {
      if (selector.startsWith('//')) {
        // XPath relative to parent
        const result = document.evaluate(
          '.' + selector.substring(selector.lastIndexOf('/')),
          parent,
          null,
    ...
    // Get all book items
    const bookElements = getElements(sel.bookItem);
    
    bookElements.forEach((book) => {
      try {
        // Extract title
        const titleEl = getElement(book, sel.title) || book.querySelector('h3 a, .title, [class*="title"]');
        const title = titleEl ? 
          (titleEl.getAttribute('title') || titleEl.textContent.trim()) : 
          'N/A';
        
        // Extract price
        const priceEl = getElement(book, sel.price) || book.querySelector('.price_color, .price, [class*="price"]');
        const price = priceEl ? priceEl.textContent.trim() : 'N/A';
        
        // Extract availability
        const availEl = getElement(book, sel.availability) || book.querySelector('.availability, [class*="stock"]');
        const availability = availEl ? availEl.textContent.trim() : 'N/A';
        
        // Extract rating if available
        const ratingEl = book.querySelector('.star-rating, [class*="rating"]');
        let rating = 'N/A';
        if (ratingEl) {
          const ratingClass = ratingEl.className;
          const match = ratingClass.match(/star-rating\s+(\w+)|rating-(\d+)/i);
          rating = match ? (match[1] || match[2]) : 'N/A';
        }
...
async function findNextPageUrl(page, selectors, currentUrl) {
  try {
    const nextUrl = await page.evaluate((sel) => {
      ...
      const nextEl = getElement(sel.nextButton) || document.querySelector('.next a, [rel="next"]');
...
async function hybridScraper() {
  ...
  log(`🔧 Method: Hybrid (AI discovery + JS extraction)\n`, "yellow");
