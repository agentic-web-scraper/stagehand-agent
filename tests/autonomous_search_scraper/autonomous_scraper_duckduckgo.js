...
function showHelp() {
  console.log(`
╔════════════════════════════════════════════════════════════════╗
║   🤖 AUTONOMOUS SCRAPER - COMMAND LINE USAGE                  ║
╚════════════════════════════════════════════════════════════════╝

USAGE:
  node autonomous_scraper_duckduckgo.js [OPTIONS]
...
EXAMPLES:
  ...
  node autonomous_scraper_duckduckgo.js --prompt "Scrape 60 books from booktoscrape.com" --format 1 --steps 50

  # Short form
  node autonomous_scraper_duckduckgo.js -p "Find React tutorials" -f 2 -s 30
...
function showWelcome() {
  console.clear();
  log("╔════════════════════════════════════════════════════════════════╗", "cyan");
  log("║   🤖 AUTONOMOUS SCRAPER WITH CUSTOM DUCKDUCKGO SEARCH 🔍      ║", "cyan");
...
// Main function
async function runCustomSearchScraper() {
  showWelcome();
  ...
  try {
    ...
    let outputFile = `custom_search_results_${Date.now()}.json`;

    // Check if user is asking for books
    if (userPrompt.toLowerCase().includes('book')) {
      selectedSchema = BooksSchema;
      outputFile = `custom_books_${Date.now()}.json`;
    } else if (formatChoice === "2") {
      selectedSchema = ArticleSchema;
      outputFile = `custom_articles_${Date.now()}.json`;
    }
    ...
    // Display summary
    log("\n" + "=".repeat(70), "cyan");
    log("🚀 STARTING AUTONOMOUS SCRAPER WITH CUSTOM SEARCH", "bright");
    log("=".repeat(70), "cyan");
...
// Run scraper
runCustomSearchScraper().catch(console.error);
