...
function showHelp() {
  console.log(`
╔════════════════════════════════════════════════════════════════╗
║   🤖 AUTONOMOUS SCRAPER WITH LIGHTPANDA - COMMAND LINE USAGE  ║
╚════════════════════════════════════════════════════════════════╝

USAGE:
  node autonomous_scraper_lightpanda.js [OPTIONS]
...
EXAMPLES:
  ...
  node autonomous_scraper_lightpanda.js

  # Non-interactive with arguments
  node autonomous_scraper_lightpanda.js --prompt "Scrape 60 books from booktoscrape.com" --format 1 --steps 50

  # Short form
  node autonomous_scraper_lightpanda.js -p "Find React tutorials" -f 2 -s 30
...
function showWelcome() {
  console.clear();
  log("╔════════════════════════════════════════════════════════════════╗", "cyan");
  log("║   🤖 AUTONOMOUS SCRAPER WITH LIGHTPANDA BROWSER 🚀            ║", "cyan");
...
async function runLightpandaScraper() {
  ...
  try {
    ...
    let outputFile = `lightpanda_search_results_${Date.now()}.json`;

    // Check if user is asking for books
    if (userPrompt.toLowerCase().includes('book')) {
      selectedSchema = BooksSchema;
      outputFile = `lightpanda_books_${Date.now()}.json`;
    } else if (formatChoice === "2") {
      selectedSchema = ArticleSchema;
      outputFile = `lightpanda_articles_${Date.now()}.json`;
    }
    ...
    // Display summary
    log("\n" + "=".repeat(70), "cyan");
    log("🚀 STARTING AUTONOMOUS SCRAPER WITH LIGHTPANDA", "bright");
    log("=".repeat(70), "cyan");
    ...
    stagehand = new Stagehand({
      ...
      localBrowserLaunchOptions: {
        cdpUrl: "ws://127.0.0.1:9222",  // Connect to Lightpanda via CDP
      },
    });
    ...
    log("⏳ Creating new page in Lightpanda...", "bright");
    const lightpandaPage = await stagehand.context.newPage();
    log("✅ Page created\n", "green");
...
- Ensure the final output contains ALL data from ALL pages visited`,
    ...
    // Execute agent with custom tools on Lightpanda page
    const result = await agent.execute({
      instruction: userPrompt,
      maxSteps: maxSteps,
      page: lightpandaPage,  // Use Lightpanda page!
    ...
    if (stagehand) {
      log("\n🔌 Closing Stagehand session...", "yellow");
...
// Run scraper
runLightpandaScraper().catch(console.error);
